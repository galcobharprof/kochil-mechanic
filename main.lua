    ...
    for k, item in pairs(items) do
        if item and item.label then
            ...
            if item.isCustom then
                detail = string.format("#%02X%02X%02X", item.r or 255, item.g or 255, item.b or 255)
            elseif item.level ~= nil then
            ...
            elseif item.isExtra then
                detail = item.isOn and "ENABLED" or "DISABLED"
            end
